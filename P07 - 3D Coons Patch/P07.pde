...
Boolean twistFree=false, animating=true, center=true, showControlPolygon=true, showFloor=false, changed = true;
float t=0, s=0;
...
void draw() {
  ...
  pushMatrix();   // to ensure that we can restore the standard view before writing on the canvas
    ...
    if(showControlPolygon) {
      ...
      fill(red);
      pts tmpP = drawBorders(P.G, 1/50f);
      fill(green); 
      pts tmpQ = drawBorders(Q.G, 1/50f); 
      fill(cyan);
      pen(blue, 5);
      if (animating) {shadeSurface(PtQ.setToL(tmpP,s,tmpQ).G, 0.05); PtQ.drawClosedLoop();}
    //fill(cyan); if(animating) {PtQ.setToL(tmpP,s,tmpQ); PtQ.drawClosedLoop(); }
...
String title ="2014: Coons patch editor & animator in 3D", name ="Shen Yang",
